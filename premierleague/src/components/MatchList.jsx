      ...
      {selectedMatch && (
        <div className="mt-8">
          <h2 className="text-2xl font-bold mb-4">
            Match Details: {selectedMatch.homeTeam.name} vs {selectedMatch.awayTeam.name}
          </h2>
          <p>Date: {new Date(selectedMatch.startTimestamp * 1000).toLocaleDateString()}</p>
          {selectedMatch.venue && (
            <p>Venue: {selectedMatch.venue.name}</p>
          )}
          {selectedMatch.homeScore && selectedMatch.awayScore && (
            <p>Score: {selectedMatch.homeScore.current} - {selectedMatch.awayScore.current}</p>
          )}
        </div>
      )}
